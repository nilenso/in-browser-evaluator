...
* [`src/cljs/in_browser_evaluator/`](src/cljs/in_browser_evaluator/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/in_browser_evaluator/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/in_browser_evaluator/`](test/cljs/in_browser_evaluator/): test files (ClojureScript,
...
7. Clone this repo and open a terminal in the `in-browser-evaluator` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/in_browser_evaluator/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns in-browser-evaluator.example
  (:require [in-browser-evaluator.config :as config])
